# ==================== 数据集配置 ====================
dataset = 'CULane'  # 定义数据集名称，使用 CULane 数据集
data_root = r'G:/Graduation_Design/DateSet/CULane' # 数据根目录，运行前需修改为实际路径
...
steps = [25, 38]  # 学习率衰减的步数，在这些步数时学习率会发生衰减
gamma = 0.1  # 学习率衰减因子，每次衰减时学习率乘以该因子
warmup = 'linear'  # 热身策略类型，采用线性热身
warmup_iters = 695  # 热身迭代次数

# ==================== 模型配置 ====================
use_aux = False  # 是否使用辅助损失
griding_num = 200  # 网格数量，用于车道线检测
...
tta = True  # 是否使用测试时间增强（Test Time Augmentation）
num_lanes = 4  # 车道线的数量
auto_backup = True  # 是否自动备份模型
num_row = 72  # 行的数量
num_col = 81  # 列的数量
train_width = 1600  # 训练图像的宽度
train_height = 320  # 训练图像的高度
num_cell_row = 200  # 行方向的单元格数量
num_cell_col = 100  # 列方向的单元格数量
fc_norm = True  # 全连接层是否使用归一化
crop_ratio = 0.6  # 裁剪比例